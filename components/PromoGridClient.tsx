...
import { motion } from 'framer-motion';          // ← обычный импорт, без TS-проблем
import { PromoBlock } from '@/types/promo';

/* ------------------ динамически тянем только Swiper ------------------ */
const Swiper = dynamic(() => import('swiper/react').then((m) => m.Swiper), {
  ssr: false,
  loading: () => (
    <div className="h-full w-full animate-pulse bg-gray-100 rounded-2xl lg:rounded-3xl" />
  ),
});
                    ...
                    <div className="relative w-full h-full aspect-[3/2]">
                      ...
                      <div className="absolute inset-0 flex flex-col justify-center items-start px-4 py-4 sm:px-12 lg:px-16 sm:py-8 lg:py-12 text-white">
                        <div className="max-w-full w-full">
                          ...
                          >
                            <span
                              className="inline-flex items-center border border-[#bdbdbd] rounded-lg px-3 sm:px-4 lg:px-6 py-1.5 sm:py-2 lg:py-3 font-bold text-xs sm:text-sm lg:text-base uppercase tracking-tight bg-white text-[#535353] shadow-sm transition-all hover:bg-[#535353] hover:text-white"
                      ...
                      <div className="absolute inset-0 flex flex-col justify-center items-start px-4 py-4 sm:px-12 sm:py-8 text-white">
                        <div className="max-w-full w-full">
                          ...
                          {item.type === 'banner' && (
                            ...
                            >
                              <span
                                className="inline-flex items-center border border-[#bdbdbd] rounded-lg px-3 sm:px-4 py-1.5 sm:py-2 font-bold text-xs sm:text-sm uppercase tracking-tight bg-white text-[#535353] shadow-sm transition-all hover:bg-[#535353] hover:text-white"
                                style={{ minWidth: 'fit-content', maxWidth: '100%' }}
                              >
          ...
          {cards.slice(0, 4).map((c, i) => (
            ...
            >
              <Link href={c.href} className="group block h-full w-full" title={c.title} role="button">
                ...
                />
                <div className="absolute inset-0 bg-black/10 group-hover:bg-black/30 rounded-2xl lg:rounded-3xl" />
