...
import { useAuth } from '@/app/context/AuthContext'; // ✅ важное: один источник истины
...
export default function StickyHeader({ initialCategories }: StickyHeaderProps) {
  ...
  const { isAuthenticated, isReady, bonus, clearAuth, refreshAuth } = useAuth();
  ...
  // ✅ micro-эффект на добавление в корзину
  useEffect(() => {
  ...
  // ✅ позиция корзины для flying animation
  useEffect(() => {
    const updatePos = () => {
  ...
  // ✅ при смене страницы - закрываем выпадашки, и можно пересинкнуть auth (на случай гонок)
  useEffect(() => {
    setOpenProfile(false);
    setIsSearchOpen(false);

    // важно: это мягкий синк, он защищен от гонок в контексте
    refreshAuth('header-pathname').catch(() => {});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pathname]);

  const handleSignOut = async () => {
    try {
      // 1) серверный logout (httpOnly cookies)
      const response = await fetch('/api/auth/logout', {
        method: 'POST',
        credentials: 'include',
      });

      // 2) очистка локального состояния (supabase + context state)
      await clearAuth();
  ...
  // ✅ ключевой момент: пока isReady=false - НЕ показываем "Вход"
  // иначе будет тот самый баг "иногда вход, потом профиль"
  const renderAuthButton = () => {
    if (!isReady) {
      // нейтральное состояние (без текста "Вход")
      return (
        <div className="p-2 rounded-full md:px-4 md:py-1 md:border md:rounded-full">
          <Image
            src="/icons/user.svg"
            alt="Профиль"
            width={20}
