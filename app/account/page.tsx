...
export const metadata: Metadata = {
  ...
  description:
    "Управляйте заказами, проверяйте бонусы и обновляйте данные в личном кабинете KEY TO HEART в Краснодаре. Вход 24/7!",
...
export default async function AccountPage() {
  // В твоих типах cookies() возвращает Promise - поэтому await обязателен
  ...
  if (phone) {
    ...
    initialBonusData = bonusesData
      ? {
          ...
          history: (bonusesData.bonus_history ?? []).map(
            (entry: any): BonusHistoryItem => ({
    ...
    initialOrders = (ordersRaw ?? []).map(
      (orderRaw: any): Order => ({
        ...
        items: (orderRaw.order_items ?? []).map(
          (item: any): OrderItem => ({
        ...
        upsell_details: Array.isArray(orderRaw.upsell_details)
          ? orderRaw.upsell_details
          : [],
      })
    );
  }
