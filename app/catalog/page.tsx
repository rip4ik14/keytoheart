...
/* ------------------------------------------------------------------ */
/*  Важно для продакшена: каталог не SSG на build (иначе ловим 60s)    */
/* ------------------------------------------------------------------ */
...
const fetchProducts = async (): Promise<Product[]> => {
  ...
  (pc || []).forEach((row) =>
    pcMap.set(row.product_id, [
      ...(pcMap.get(row.product_id) || []),
      row.category_id,
    ])
  );
  ...
  (ps || []).forEach((row) =>
    psMap.set(row.product_id, [
      ...(psMap.get(row.product_id) || []),
      row.subcategory_id,
    ])
  );
  ...
  return (pr || []).map((p) => ({
    ...
    subcategory_names: (psMap.get(p.id) || []).map(
      (id) => subNameMap.get(id) || ""
    ),
  }));
};
