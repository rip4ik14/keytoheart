...
import type { Database, Json } from '@/lib/supabase/types_new'; // Добавляем импорт Json

// Тип для сырых данных из Supabase
interface RawDaySchedule {
  start: string;
  end: string;
}

// Тип для преобразованных данных с полем enabled
interface DaySchedule extends RawDaySchedule {
  enabled: boolean;
}

interface StoreSettings {
  id: number; // Сделаем id обязательным, так как оно должно быть в базе
...
// Инициализация расписания по умолчанию для каждого дня
...
const defaultSettings: StoreSettings = {
  id: 0, // Предполагаем, что id будет заменён реальным значением из базы
...
// Функция для безопасного преобразования Json в Record<string, DaySchedule>
...
export default function AdminSettingsPage() {
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      console.log('Authenticated user:', userError ? `Error: ${userError.message}` : user);

      console.log('Attempting to fetch settings from store_settings...');
      ...
      if (!data) {
        throw new Error('Настройки магазина не найдены');
      }

      // Преобразуем данные, добавляя enabled и нормализуя поля
      ...
      toast.error('Ошибка загрузки настроек магазина: ' + (error.message || 'Неизвестная ошибка'));
  ...
  const handleSave = async () => {
    ...
    if (settings.id === undefined) {
      toast.error('Ошибка: ID настроек не определён');
      return;
    }
    ...
    try {
      ...
      if (error) throw new Error(`Ошибка сохранения: ${error.message} (код: ${error.code})`);

      setOriginalSettings(settings);
      toast.success('Настройки сохранены');
      window.gtag?.('event', 'save_store_settings', {
        event_category: 'admin',
        banner_active: settings.banner_active,
      });
      window.ym?.(12345678, 'reachGoal', 'save_store_settings', {
        banner_active: settings.banner_active,
      });
    } catch (error: any) {
  ...
  const handleScheduleChange = (
    ...
    setSettings((prev) => {
      ...
      if (field === 'start' || field === 'end') {
        const start = field === 'start' ? (value as string) : updatedSchedule[day].start;
        const end = field === 'end' ? (value as string) : updatedSchedule[day].end;
  ...
  const copyScheduleToAllDays = (type: 'order_acceptance_schedule' | 'store_hours', sourceDay: string) => {
    setSettings((prev) => {
      if (!prev) return prev;
      const sourceSchedule = prev[type][sourceDay];
      const updatedSchedule = daysOfWeek.reduce((acc, day) => {
        acc[day.key] = {
    ...
    toast.success(`Расписание скопировано на все дни для ${type === 'order_acceptance_schedule' ? 'приёма заказов' : 'графика доставки'}`);
  };
    ...
    >
        ...
        <div className="flex items-center gap-4 mb-4">
          <label className="flex items-center gap-2">
            <input
              ...
              className="form-checkbox h-5 w-5 text-black focus:ring-2 focus:ring-offset-2 focus:ring-black"
              aria-label="Включить приём заказов"
            />
            <span className="text-sm text-gray-600">Приём заказов включён</span>
          </label>
          <motion.button
            onClick={enable24HourOrderAcceptance}
            className="bg-gray-200 text-gray-800 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-300 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black"
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            aria-label="Включить приём заказов 24/7"
          >
            Включить 24/7
          </motion.button>
        </div>
        ...
        {daysOfWeek.map((day) => (
          ...
          >
            ...
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={settings.order_acceptance_schedule[day.key].enabled}
                onChange={(e) =>
                  handleScheduleChange('order_acceptance_schedule', day.key, 'enabled', e.target.checked)
                }
                className="form-checkbox h-5 w-5 text-black focus:ring-2 focus:ring-offset-2 focus:ring-black"
                aria-label={`Включить приём заказов в ${day.label}`}
              />
            ...
            <div className="flex gap-2 items-center">
              <input
                type="time"
                value={settings.order_acceptance_schedule[day.key].start}
                onChange={(e) =>
                  handleScheduleChange('order_acceptance_schedule', day.key, 'start', e.target.value)
                }
                disabled={!settings.order_acceptance_schedule[day.key].enabled}
                className={`border border-gray-300 rounded-lg px-3 py-1 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black ${
                  !settings.order_acceptance_schedule[day.key].enabled ? 'opacity-50 cursor-not-allowed' : ''
                }`}
                aria-label={`Начало приёма заказов в ${day.label}`}
              />
              <span className="text-gray-500">-</span>
              <input
                type="time"
                value={settings.order_acceptance_schedule[day.key].end}
                onChange={(e) =>
                ...
                aria-label={`Конец приёма заказов в ${day.label}`}
              />
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                aria-label={`Скопировать расписание приёма заказов для ${day.label} на все дни`}
              >
        ...
        {daysOfWeek.map((day) => (
          ...
          >
            ...
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={settings.store_hours[day.key].enabled}
                onChange={(e) =>
                  handleScheduleChange('store_hours', day.key, 'enabled', e.target.checked)
                }
                className="form-checkbox h-5 w-5 text-black focus:ring-2 focus:ring-offset-2 focus:ring-black"
                aria-label={`Включить доставку в ${day.label}`}
              />
              <span className="text-sm text-gray-600">Включён</span>
            </label>
            <div className="flex gap-2 items-center">
              <input
                type="time"
                value={settings.store_hours[day.key].start}
                onChange={(e) =>
                  handleScheduleChange('store_hours', day.key, 'start', e.target.value)
                }
                disabled={!settings.store_hours[day.key].enabled}
                className={`border border-gray-300 rounded-lg px-3 py-1 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black ${
                  !settings.store_hours[day.key].enabled ? 'opacity-50 cursor-not-allowed' : ''
                }`}
                aria-label={`Начало доставки в ${day.label}`}
              />
              <span className="text-gray-500">-</span>
              <input
                type="time"
                value={settings.store_hours[day.key].end}
                onChange={(e) =>
                ...
                aria-label={`Конец доставки в ${day.label}`}
              />
              <motion.button
                ...
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                aria-label={`Скопировать расписание доставки для ${day.label} на все дни`}
              >
                <Image src="/icons/copy.svg" alt="Скопировать" width={16} height={16} />
              </motion.button>
            </div>
      ...
      >
        ...
        <label className="flex items-center gap-2 mb-4">
          <input
            ...
            className="form-checkbox h-5 w-5 text-black focus:ring-2 focus:ring-offset-2 focus:ring-black"
            aria-label="Активировать баннер временного закрытия"
          />
        ...
        <div className="mb-4">
          ...
          <textarea
            ...
            className="w-full border border-gray-300 rounded-lg p-2 min-h-[80px] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black"
            aria-label="Введите текст баннера"
          />
        </div>
      ...
      <motion.div className="sticky bottom-4 bg-white pt-4 border-t flex gap-4" variants={itemVariants}>
        <motion.button
          ...
          className={`flex-1 bg-black text-white py-3 rounded-lg font-medium hover:bg-gray-800 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black ${
            isSaving || !hasUnsavedChanges ? 'opacity-50 cursor-not-allowed' : ''
          }`}
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
          aria-label="Сохранить настройки магазина"
        >
          {isSaving ? (
        ...
        <motion.button
          ...
          disabled={isSaving || !hasUnsavedChanges}
          className={`flex-1 bg-gray-200 text-gray-800 py-3 rounded-lg font-medium hover:bg-gray-300 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black ${
            isSaving || !hasUnsavedChanges ? 'opacity-50 cursor-not-allowed' : ''
          }`}
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
