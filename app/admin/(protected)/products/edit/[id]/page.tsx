...
const SortableImage = ({ image, onRemove, isExisting, url }: { image?: ImageFile; onRemove: (id: string) => void; isExisting: boolean; url?: string }) => {
  const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: image?.id || url! });
  const style = { transform: CSS.Transform.toString(transform), transition };

  return (
    <motion.div ref={setNodeRef} style={style} className="relative" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
      <div className="relative w-24 h-24">
        {isExisting ? (
          <Image src={url!} alt="Existing Image" fill className="object-cover rounded-lg" />
        ) : (
          <Image src={URL.createObjectURL(image!.file)} alt={image!.file.name} fill className="object-cover rounded-lg" />
        )}
        <button
...
export default function EditProductPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        router.push(`/admin/login?from=${encodeURIComponent(`/admin/products/edit/${id}`)}`);
      }
    };
    checkAuth();
  }, [router, id]);
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newImages = files.map((file) => ({ id: uuidv4(), file }));
  ...
  const handleRemoveImage = async (id: string, isExisting: boolean) => {
    if (isExisting) {
      try {
        ...
        const { error } = await supabase.storage.from('product-image').remove([fileName]);
  ...
  const compressImage = (file: File): Promise<File> => {
    return new Promise((resolve, reject) => {
      new Compressor(file, {
        ...
        error: (err) => reject(new Error('Ошибка сжатия изображения: ' + err.message)),
      });
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent, csrfToken: string) => {
    ...
    try {
      ...
      const productId = parseInt(id, 10);
      if (isNaN(productId)) throw new Error('Неверный ID товара');
      ...
      if (originalPrice && (isNaN(originalPriceNum) || originalPriceNum <= 0)) throw new Error('Старая цена должна быть > 0');
      ...
      if (productionTime && (isNaN(productionTimeNum!) || productionTimeNum! < 0)) throw new Error('Время изготовления должно быть ≥ 0');
      const bonusNum = bonus ? parseFloat(bonus) : 0;
      if (bonus && (isNaN(bonusNum) || bonusNum < 0)) throw new Error('Бонус должен быть ≥ 0');

      const { data: categoriesData, error: categoriesError } = await supabase
        .from('categories')
        .select('name, slug')
        .in('id', categoryIds);
      if (categoriesError || !categoriesData) throw new Error('Ошибка получения категорий: ' + categoriesError.message);

      const categoryNames = categoriesData.map((cat) => cat.name).join(', ');
      const categorySlugs = categoriesData.map((cat) => cat.slug);
      if (isPopular && categorySlugs.some((slug) => ['balloon', 'postcard'].includes(slug))) throw new Error('Шарики и открытки нельзя сделать популярными товарами');
      ...
      if (images.length > 0) {
        for (const image of images) {
          ...
          const { error } = await supabase.storage.from('product-image').upload(fileName, compressedImage);
          if (error) throw new Error('Ошибка загрузки изображения: ' + error.message);

          const { data: publicData } = supabase.storage.from('product-image').getPublicUrl(fileName);
          if (publicData?.publicUrl) imageUrls.push(publicData.publicUrl);
          else throw new Error('Не удалось получить публичный URL изображения');
        }
      }
  ...
  if (isAuthenticated === null || !product) return <div className="min-h-screen flex items-center justify-center">Загрузка...</div>;
  if (!isAuthenticated) return null;

  return (
    <CSRFToken>
      {(csrfToken) => (
        <motion.div className="max-w-6xl mx-auto p-6 space-y-6 bg-gray-50 rounded-lg shadow-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
          ...
          <form onSubmit={(e) => handleSubmit(e, csrfToken)} className="space-y-8">
                    ...
                    {filteredSubcategories.map((sub) => (
                      <label key={sub.id} className="flex items-center">
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) setSubcategoryIds((prev) => [...prev, sub.id]);
                            else setSubcategoryIds((prev) => prev.filter((id) => id !== sub.id));
                          }}
                          className="mr-2"
                        />
            ...
            <section className="space-y-4 bg-white p-6 rounded-lg shadow-sm">
                    ...
                    <SortableContext items={[...existingImages, ...images.map((img) => img.id)]}>
                      <div className="grid grid-cols-3 gap-4 mb-4">
                        {existingImages.map((url) => (
                          <SortableImage key={url} url={url} onRemove={(id) => handleRemoveImage(id, true)} isExisting={true} />
                        ))}
                        {images.map((image) => (
                          <SortableImage key={image.id} image={image} onRemove={(id) => handleRemoveImage(id, false)} isExisting={false} />
                        ))}
                      </div>
                    </SortableContext>
                  </DndContext>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="inStock" className="font-medium text-gray-600">В наличии</label>
              </div>
              <div className="flex items-center gap-2">
                <input
                  id="isVisible"
                  ...
                  className="w-5 h-5 text-black focus:ring-black border-gray-300 rounded"
                />
                <label htmlFor="isVisible" className="font-medium text-gray-600">Показать товар</label>
              </div>
              <div className="flex items-center gap-2">
                <input
                  id="isPopular"
                  ...
                  className="w-5 h-5 text-black focus:ring-black border-gray-300 rounded"
                />
                <label htmlFor="isPopular" className="font-medium text-gray-600">Популярный товар</label>
              </div>
            </section>
