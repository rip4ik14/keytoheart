...
export default function NewProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent, csrfToken: string) => {
    ...
    try {
      ...
      if (originalPrice && (isNaN(originalPriceNum) || originalPriceNum <= 0)) throw new Error('Старая цена должна быть > 0, если указана');
      ...
      const { data: categoriesData, error: categoriesError } = await supabase
        ...
        .select('name, slug')
        .in('id', categoryIds);
      if (categoriesError || !categoriesData) throw new Error('Ошибка получения категорий: ' + (categoriesError?.message || 'Неизвестная ошибка'));
