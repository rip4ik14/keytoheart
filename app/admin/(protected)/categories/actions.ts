...
export async function updateCategory(formData: FormData) {
  ...
  const { data: existing, error: exErr } = await supabase
    .from("categories")
    .select(
      "name,slug,is_visible,seo_h1,seo_title,seo_description,seo_text,og_image,seo_noindex,home_is_featured,home_sort,home_title",
    )
    .eq("id", id)
    .single();
  ...
  let slug =
    slugRaw !== null
      ? String(slugRaw ?? "").trim()
      : String((existing as any).slug ?? "").trim();
  ...
  if (vHomeFeatured !== null)
    payload.home_is_featured = cleanBool(vHomeFeatured, false);

  const vHomeSort = formData.get("home_sort");
  if (vHomeSort !== null)
    payload.home_sort = Math.max(0, cleanInt(vHomeSort, 0));
...
export async function updateSubcategory(formData: FormData) {
  ...
  if (!category_id)
    throw new Error("Не удалось определить category_id подкатегории");
