    ...
    const fetchProduct = async () => {
      try {
        const { data, error } = await supabasePublic
          .from('products')
          .select('id, title, price, original_price, short_desc, description, composition, images, in_stock, is_visible, is_popular, discount_percent')
          .eq('id', parseInt(id))
          .single();
  ...
  const handleSubmit = async (e: React.FormEvent, csrfToken: string) => {
    ...
    try {
      ...
      if (categoriesError || !categoriesData) {
        throw new Error('Ошибка получения названий категорий: ' + categoriesError.message);
      }
