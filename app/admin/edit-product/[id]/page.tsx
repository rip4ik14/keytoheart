...
interface Product {
  ...
  category: string;
  category_id: number | null;
  subcategory_id: number | null; // Убираем опциональность, так как нормализуем в null
  short_desc: string;
  description: string;
  composition: string;
  images: string[];
...
export default function EditProductPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        console.log('Auth check response in EditProduct:', data);
        ...
        console.error('Auth check error in EditProduct:', err.message);
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        // Нормализуем данные перед установкой в состояние
        const normalizedData: Product = {
          ...data,
          category: data.category ?? '',
          discount_percent: data.discount_percent !== undefined ? data.discount_percent : null,
          subcategory_id: data.subcategory_id ?? null, // Гарантируем, что это number | null
        ...
        // Проверяем, что category_id не null, так как это обязательное поле
        if (normalizedData.category_id === null) {
          throw new Error('Категория товара не указана');
        }
  ...
  useEffect(() => {
    if (categoryId) {
      ...
      // Используем значение из product, если product не null
      if (product) {
        setSubcategoryId(product.subcategory_id); // Теперь это number | null
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent, csrfToken: string) => {
    ...
    try {
      ...
      if (isPopular && ['balloon', 'postcard'].includes(categoryData.slug)) {
        throw new Error('Шарики и открытки нельзя сделать популярными товарами');
      }

      const categoryName = categoryData.name;

      // Загрузка новых изображений в Supabase Storage
