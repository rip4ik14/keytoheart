...
export async function GET(_req: NextRequest) {
  // Важно: это Node runtime (по умолчанию для route.ts), Supabase ок
  ...
  if (catErr) {
    // Логика на случай ошибки БД: всё равно отдаем статику, чтобы не падать
    console.error('Sitemap categories error:', catErr);
  }
  const cats = (cat ?? []).map((c) => tag(`${BASE}/category/${c.slug}`));

  /* 3. Товары */
  const { data: pr, error: prodErr } = await supabase
    .from('products')
    .select('id')
    .eq('is_visible', true)
    .eq('in_stock', true);
  ...
  if (prodErr) {
    console.error('Sitemap products error:', prodErr);
  }
