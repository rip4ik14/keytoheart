import './styles/globals.css';
   ...
   type Category = {
     ...
     name: string;
     slug: string;
     subcategories: { id: number; name: string; slug: string; is_visible: boolean | null }[];
   };
       ...
       categories = Array.isArray(data)
         ? data.map((cat) => ({
             ...
             subcategories: cat.subcategories
               ? cat.subcategories.filter((sub) => sub.is_visible === true) // Фильтруем только видимые подкатегории
               : [],
           }))
         : [];
     } catch (err) {
